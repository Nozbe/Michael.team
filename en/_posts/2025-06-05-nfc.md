...
### Step 1. Shortcut to toggle the web cam.

Just go to Shortcuts app, *create new Shortcut*, add action called *Toggle Accessory or Scene*, choose the accessory (which in my case was the smart plug called "Sony") and save the Shortcut as "Webcam Toggle".

![{{ page.title }} sony](/img/nfc-sony.png)

### Step 2. Configure NFC Tag.

Go to *Automation* in the Shortcuts app (NOT Home app!), hit ➕ to add new automation and find the *NFC* option, and configure the NFC tag, give it a name, set it to *run immediately* and choose to invoke the above-configured "Webcam Toggle" Shortcut.

![{{ page.title }} scan](/img/nfc-scan.png)

### Stwp 3. There is no step 3! 🤪

Now that when you touch the unlocked iPhone next to the NFC tag, the "Webcam Toggle" will be triggered.
...
A few months after I wrote this post I needed to update Homebridge and in the UI the updates failed, so I checked and I needed to run these commands in Terminal:

1. `sudo npm install -g homebridge@latest` - to update Homebridge
2. `sudo npm install -g homebridge-config-ui-x@latest` - to update its UI
3. `sudo hb-service update-node` - to update node

And later I could verify the version by typing `homebridge -V` in Terminal. That's it.
